...
type NewProp = { name: Prop, type: SQLColumnType, isPrimary?: boolean, nullable?: boolean, isUnique?: boolean, default?: SQLType, autoIncrement?: boolean};
...
& (T extends undefined ? {} :
  (T extends { isPrimary?: true } ? {} : P extends true ? {} : { PRIMARY: ()=>Return<T & { isPrimary: true }, true> })
  ...
  & (T extends { type: "INT" }
      ? (T extends { autoIncrement: true } ? {} : { AUTO_INCREMENT: () => Return<T & { autoIncrement: true }, P> }) : {})
  );

export function CREATE<T extends NewProp | undefined, P extends boolean = false>(table: Table, ifNotExists: boolean, props: NewProp[] = [], currentProp?: T | undefined): Return<T, P> {
  ...
  return {
    ...
    newDECIMAL: (name: string, precision: number, scale: number) => nextProp({name, type: `DECIMAL(${precision},${scale})`}),
