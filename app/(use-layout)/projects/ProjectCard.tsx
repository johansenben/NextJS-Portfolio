...
function ProjectCardTemplate({children, className}: {children: ReactNode, className?: string}) {
  return (
    <li className={clsx("border-4 border-solid m-2 p-4 relative", className ?? "")}>
      {children}
    </li>
  );
}
function Status({label, Icon, className}: {label: string, Icon: React.ComponentType<React.SVGProps<SVGSVGElement>>, className?: string}) {
  return (
...
export function ProjectCard({ name, description, href, image }: { name: string, description: string, href: string, image: string }) {
  return (
