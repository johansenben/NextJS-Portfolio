	...
	return (
		<div className={clsx("border-2 grid grid-cols-4 p-2 my-2", index >= 2 && !expanded ? "hidden print:block" : "")}>
			...
			<div className="col-start-4 row-start-1 row-span-2 print:row-start-3 print:col-start-1">
				{starts.map((start, i) =>
					ends[i] ? (
						<h5 className="font-light text-sm text-right print:text-xs print:text-left" key={`exp-${title}-${start}`}>
							{start} - {ends[i]}
						</h5>
					) : (
						""
	...
	return (
		<div>
			...
			{experience.map((exp, i) => (
				<ExperienceCard {...exp} index={i} expanded={expanded} key={`experience-card-${i}`} />
			))}
			<button
...
export function Education({ education }: { education: EduProps[] }) {
	...
	return (
		<div>
			...
			{education.map((edu, i) => (
				<EducationCard {...edu} index={i} expanded={expanded} key={`education-card-${i}`} />
			))}
			<button
...
function Skill({ children, logoName }: { children: ReactNode; icon?: string; logoName?: string }) {
	return (
...
export function Skills({ skills }: { skills: { programming: string[]; IT: string[]; other: string[] } }) {
	return (
		<div>
